...
- feat(debug): 8 new step files under `skills/debug/`:
  ...
  - `step-04-fix.md` — write the minimal fix addressing the confirmed root cause, **inlined Failure Handling** (3-attempt retry rule) + **inlined Circuit Breaker** (5+ failed attempts → `ops-circuit-breaker` process + escalate to user, do NOT mark task completed).
...
- No behavior change: the 8-step workflow, the Iron Law, the max-3-hypotheses rule, the git-historian Investigation Mode dispatch, the circuit breaker trigger, the trivial fix exception, and the downstream expectation (show evidence before declaring fixed) all behave identically to v3.6.2. Only the file layout, the Iron Law framing as HARD-GATE, and the addition of task tracking changed.
