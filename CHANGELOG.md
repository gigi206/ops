...
Prior releases mentioned LSP once, as a soft bullet in `ops-subagent-rules`: *"Use LSP tools when available."* No skill enforced it. No checklist verified it. No agent except `code-reviewer` and `pr-reviewer` (diagnostics only) had explicit LSP instructions. Result: agents defaulted to grep and to dispatching `researcher-code` for symbol-oriented queries that LSP answers in milliseconds — burning tokens on questions that have deterministic answers.

This release promotes LSP usage from a soft hint to a HARD-GATE with enforcement at both the dispatcher side (the skills that spawn agents) and the agent side (the agents that read and modify code). The gate has explicit scope (symbol-oriented queries), explicit exceptions (semantic questions, structural questions, LSP unavailable), and explicit fallback semantics (grep/dispatch when LSP cannot answer). Scope is intentionally targeted: four agents and three skills get LSP-first instructions, the two agents that already had LSP diagnostics keep their existing contract unchanged, and the agents that do not read code (researcher-doc, git-historian, critic) are not touched.

**Why now**: v3.11.0 shipped ast-grep integration for the plan Duplication Scan. LSP is the natural companion — LSP answers "what symbols exist" in milliseconds, ast-grep answers "what body shapes match", the manual pass answers "is this real duplication or coincidence". The three tools compose cleanly. Integrating ast-grep without simultaneously promoting LSP would leave the duplication inventory half-automated — LSP would be the cheapest first-pass probe and it was not being invoked.

- feat(subagent-rules): new `<HARD-GATE-LSP>` block replaces the soft "Use LSP tools when available" bullet. Mandates LSP-first for five categories of symbol-oriented queries (goToDefinition, findReferences, documentSymbol, workspaceSymbol, hover), lists the four allowed dispatch exceptions (LSP unavailable, LSP fails, semantic question, structural question), specifies the fallback protocol (note once, fall back, do not retry), and explicitly forbids burning researcher-code dispatches on symbol lookups LSP answers in milliseconds. Explicit guidance: `findReferences` is MANDATORY before any function signature change, `goToDefinition` is the first probe for stack-trace debugging, `documentSymbol` is the first probe for duplication inventory.
- feat(agents/researcher-code): new Protocol Step 0 "LSP-first for symbol-oriented sub-questions". The agent is explicitly told to use `goToDefinition`, `findReferences`, `documentSymbol`, `workspaceSymbol`, `hover` first for symbol-oriented sub-questions encountered during exploration, and to fall back to grep only when LSP is unavailable or the question is semantic/structural. Step 1 "Map the structure" now uses `documentSymbol` before full-file reads to get the symbol inventory without token cost.
- feat(agents/implementer): Step 3 "Read Before Write" extended with LSP-assisted read. `documentSymbol` for symbol inventory before full read; `goToDefinition` for cross-file symbol references; `findReferences` before modifying any function's signature or behavior, with a clause that reports DONE_WITH_CONCERNS if the blast radius exceeds what the plan described. LSP unavailability fallback documented.
- feat(agents/security-reviewer): Step 1 "Scope Assessment" extended with MANDATORY `findReferences` on security-critical functions whose signature or behavior changes in the diff. The caller set is the trust-boundary analysis surface — grep-based tracing misses aliases, re-exports, and dynamic invocations that LSP resolves correctly. Fallback to grep documented as lossy for security purposes.
- feat(agents/test-writer): Step 2 "Understand the Code" extended with `findReferences` on each target function to inventory real usage patterns. The reference sites shape the edge-case list: inputs used in practice, not inputs imagined. Pairs with `documentSymbol` for the file-level symbol list. Fallback documented.
- feat(refactor): Step 4 "Plan Incremental Steps" gets a new `<HARD-GATE-REFACTOR-LSP>` block. `findReferences` is MANDATORY before planning any step that changes a function/method/class signature (add/remove/reorder parameters, change type, change return type, rename, remove). Step descriptions must include the reference count and affected callers. Non-signature refactorings (rename-within-scope, extract-local, inline-local, file-internal move) use `documentSymbol` instead. Explicit degraded-mode wording when LSP is unavailable: "falling back to grep-based reference scan. Blast radius may be incomplete."
- feat(debug): Step 1 "Investigate" — new sub-step 3 "LSP-first symbol resolution on the stack trace". For each stack trace frame, run `goToDefinition` before dispatching git-historian or grepping for the error site. `findReferences` on the error-site symbol to build the caller-set hypothesis surface. `hover` for unfamiliar types. End-of-step checklist updated to verify the LSP resolution ran before git-historian dispatch.
- feat(plan): Step 7 Duplication Scan "Tool-assisted scan" sub-section restructured as a two-tier pipeline. **Tier 1**: LSP `documentSymbol` for cheap symbol inventory (first probe, free, fast, symbol-precise). **Tier 2**: ast-grep for body-shape matching (where LSP cannot answer). The two tools compose — LSP narrows the candidate set to "symbols that plausibly overlap", ast-grep confirms with a structural body comparison, manual pass decides "real duplication vs coincidence". Neither replaces the other.
- feat(implement): Step 2c "Conformity Check" adds a new per-task LSP diagnostics checkbox. Run LSP diagnostics on every file the task modified immediately after the conformity check. Errors introduced by the current task → blocker (implementer fixes before next task). Pre-existing errors → noted in the task record but not a blocker. LSP unavailable → skip with a note, rely on final-review diagnostics. Rationale: a type error introduced in Task 1 and detected only at Task N's final review means N-1 tasks were built on a broken assumption. Per-task LSP diagnostics close the feedback loop to one task.
- docs(changelog): this release explicitly calls out that the two agents already using LSP (code-reviewer, pr-reviewer, both for diagnostics only) keep their existing contract unchanged. The three agents that do not read code (researcher-doc, git-historian, critic) are deliberately not touched. researcher-repo is also not touched because external repositories are rarely LSP-configured on fresh clones. Deferred scope is explicit: extending code-reviewer / pr-reviewer beyond diagnostics (navigation during review) is judged low-value-per-line and would expand their calibrated effort baselines.

Scope: 10 substantive content files (1 subagent-rules, 4 agents, 4 skills, 1 CHANGELOG) plus 4 support files (README sync per the v3.10.0 rule + 3 version files). Version: 3.11.0 → 3.12.0 (minor — new enforcement capability, observable behavior change on weak models that previously defaulted to grep for symbol queries).

## 3.11.0 (2026-04-11)

### ast-grep integration — tool-assisted Duplication Scan

The v3.10.0 plan Step 7 Duplication Scan relies on the planner doing a pairwise comparison of new logic across all plan tasks **mentally**. This works on strong reasoning models but breaks down on longer plans (> 5 tasks) and on weaker models reached via OpenCode providers, where structural comparison is unreliable. `ast-grep` is a structural code search and rewrite tool that operates on AST patterns, cross-language, with no LSP server required. Using it to inventory existing shapes in the files a plan will touch turns the cognitive exercise into a concrete command with deterministic output.

This release adds ast-grep as an **optional** ops tool (same tier as qlty and semgrep) with a targeted integration in the Duplication Scan. The manual pairwise path remains canonical — ast-grep is a force multiplier, not a replacement — so plans written without ast-grep installed continue to work identically. Scope is intentionally narrow: init detection + Step 7 tool-assisted section + Requirements doc. Broader integration (research / refactor / debug skills using ast-grep as a cheaper alternative to researcher-code dispatches for pure structural queries) is deferred to a future release once the Duplication Scan integration has been validated in practice.

- feat(init): Step 2c — new ast-grep detection section in `skills/init/step-02-ops-tools.md`. Check `which ast-grep` (or the short alias `sg`), report version as installed or missing, classify as **optional** (not a blocker for stop-and-propose, unlike qlty and semgrep). Install hints added for brew, cargo, npm, and pip package managers — only the commands matching package managers detected in Step 0c are shown to the user. Summary block gains an `- ast-grep: <status>` line. End-of-step checklist extended to verify the detection ran.
- feat(plan): Step 7 Duplication Scan — new "Tool-assisted scan (if ast-grep is available)" sub-section. Documents when to use it (Normal light pass, Complex full pass — skip in Simple), how to use it (`command -v ast-grep` check first, then 1-3 of the starter patterns per file the tasks touch), and what to do with the results (ast-grep gives concrete `file:line` match locations, domain judgement stays manual). Three starter patterns provided as stack-agnostic templates: (1) functions by body shape, (2) handlers reacting to the same event / calling the same mutation, (3) calls to the same external API with the same serialization. Each pattern includes adaptation guidance for different language forms (JS arrows, Python def, language-specific handler shapes). Fallback behavior explicit: if ast-grep is not installed, fall back to the manual scan algorithm and note `"Duplication scan: manual only (ast-grep not installed…)"` in the plan's Risks section so the critic can calibrate Lens 5 scrutiny.
- refactor(plan): Step 7 End-of-step checklist — Duplication Scan checkbox extended. When ast-grep was used, the Risks note SHOULD mention it (`"Duplication scan: clean (N tasks compared, ast-grep assisted)"`). When ast-grep was not available, the Risks note SHOULD mention that too, so the plan-stage critic can see which path was taken and apply more or less Lens 5 scrutiny accordingly.
- docs(readme): Requirements section adds an `ast-grep` optional tool entry with the same four package manager install commands as init Step 2c, a one-sentence rationale for why it matters, and an explicit note on the weaker-model value proposition for OpenCode users. `/ops-plan` skill step table Duplication Scan row extended to mention the tool-assisted path and the three starter patterns.
- docs(plan): the Duplication Scan section explicitly states that ast-grep is a **force multiplier**, not a gate. Plans without ast-grep installed run the manual canonical path and pass the end-of-step checklist identically. This keeps the optional-tool contract consistent with qlty and semgrep (strongly recommended but not blocking) and with the project's stack-agnostic philosophy (no dependency on a specific tooling ecosystem).

Note on rationale weighting: the changelog explicitly calls out that ast-grep's value is disproportionately high on weaker models. On strong reasoning models the manual pairwise comparison is reliable enough that ast-grep is a nice-to-have; on weaker models (smaller open-weight models, various OpenCode providers) the manual comparison breaks down past ~5 tasks and ast-grep becomes load-bearing for scan reliability. The integration is designed so both audiences get the same fallback path (manual) when ast-grep is absent, while users who install it get a deterministic alternative that scales with plan length.
