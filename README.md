...
| `ops-subagent-rules`          | Context rules for dispatching subagents (inline content, scoping, labeling) + **HARD-GATE-LSP**: LSP-first for symbol-oriented queries (`goToDefinition`, `findReferences`, `documentSymbol`, `workspaceSymbol`, `hover`). Dispatching `researcher-code` for a symbol lookup LSP answers in milliseconds is a FAILURE. Exceptions: semantic questions, structural questions (ast-grep territory), and LSP-unavailable fallback. | plan, implement, do, debug, research, clone-analyze, test, refactor, perf, review-pr  |
...
| :-------------------- | :-------------: | :----------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| **researcher-code**   |   opus (high)   | Explore codebase: patterns, conventions, implementations, integration points, risks. **LSP-first** for symbol-oriented sub-questions (`documentSymbol` inventory before full-file reads, `goToDefinition` / `findReferences` before grep) per HARD-GATE-LSP. | research, do, test, refactor, perf, implement (circuit-breaker), debug (circuit-breaker) |
...
| **implementer**       |   opus (high)   | Execute one plan task (TDD, code generation, validation). **LSP-assisted read**: `documentSymbol` for symbol inventory, `goToDefinition` for cross-file references, MANDATORY `findReferences` before modifying any function signature — reports DONE_WITH_CONCERNS if blast radius exceeds plan. | implement                                                                                |
...
| **security-reviewer** |   opus (high)   | Deep security analysis: code, infra, CI/CD, containers, supply chain. **LSP blast-radius tracing**: MANDATORY `findReferences` on security-critical functions whose signature or behavior changes in the diff — grep-based tracing misses aliases, re-exports, dynamic invocations that LSP resolves correctly. | security-gate: implement, do, debug, security, review-pr                                 |
| **test-writer**       |   opus (high)   | Analyze existing code and write tests: behavior analysis, edge cases, coverage. **LSP caller inventory**: `findReferences` on each target function to shape the edge-case list from real usage sites — inputs used in practice, not inputs imagined. | test                                                                                     |
...
| 6     | LSP — CLI-specific diagnostic (Claude Code: 4-level, OpenCode: binary+config). **Load-bearing for HARD-GATE-LSP**: this diagnostic feeds 4 agents and 4 skills that use LSP as their first probe for symbol-oriented queries. When a downstream skill sees "LSP not available for language X", it is this phase's output being consulted. | Yes             |
...
- **Adversarial review** — the `critic` agent tries to break your plan before you build it. Two review modes: **PLAN** (full 5-lens review, dispatched from `/ops-plan`) and **BRAINSTORM** (reduced invariant-class check, dispatched from `/ops-brainstorm` Step 11 on a signal-gated basis). Brainstorm Step 7 is protected by a pair of gates: `HARD-GATE-NEUTRALITY` forbids pre-picking answers (with an explicit exception for invariant-class decisions), and `HARD-GATE-ANTIPATTERNS` renders three concrete violation shapes observed in weaker-model runs (fake `(Recommended)` tags without Step 1-6 context citation, fabricated strawman cons on the non-preferred approach, batching the macro-approach question with dimension questions) so the agent can pattern-match its own draft against forbidden templates rather than reasoning abstractly about neutrality.
...
- **LSP-first for symbol queries** — `HARD-GATE-LSP` in `ops-subagent-rules` mandates that `goToDefinition`, `findReferences`, `documentSymbol`, `workspaceSymbol`, and `hover` are the first probe for any symbol-oriented question, **before** grep or agent dispatch. Burning tokens on questions LSP answers in milliseconds is a FAILURE. Applies across `researcher-code` (exploration), `implementer` (read-before-write), `security-reviewer` (blast-radius tracing), `test-writer` (caller inventory), `/ops-refactor` (mandatory `findReferences` before signature changes), `/ops-debug` Step 1 (stack-trace symbol resolution), `/ops-plan` Duplication Scan (Tier 1 `documentSymbol` + Tier 2 ast-grep), and `/ops-implement` Step 2c (per-task LSP diagnostics). Fallback to grep is allowed only when LSP is genuinely unavailable.
